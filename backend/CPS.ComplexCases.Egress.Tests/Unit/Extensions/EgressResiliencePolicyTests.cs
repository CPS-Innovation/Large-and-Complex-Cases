...
namespace CPS.ComplexCases.Egress.Tests.Unit.Extensions;

public class EgressResiliencePolicyTests
{
  ...
  [Fact]
  public async Task CircuitBreaker_DoesNotOpenForRateLimitResponses()
  {
    var policy = CreateBreaker();

    for (var i = 0; i < MinimumThroughput * 2; i++)
    {
      await policy.ExecuteAsync(() => Respond(HttpStatusCode.TooManyRequests));
    }

    var response = await policy.ExecuteAsync(() => Respond(HttpStatusCode.TooManyRequests));
