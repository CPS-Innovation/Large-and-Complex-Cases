...
public class S3CredentialService : IS3CredentialService
{
    ...
    public async Task<(string? accessKey, string? secretKey)> GetCredentialKeysAsync(string bearerToken)
    {
        ...
        {
            throw new InvalidOperationException("preferred_username claim is missing in the bearer token.");
        }

        if (string.IsNullOrEmpty(oid))
        {
            throw new InvalidOperationException("oid claim is missing in the bearer token.");
        }

        var credentials = await GetCredentialsAsync(oid, username, bearerToken);

        return (credentials.AccessKey, credentials.SecretKey);
    }

    public async Task<(string? accessKey, string? secretKey)> RegenerateCredentialKeysAsync(string bearerToken)
    {
        var handler = new JwtSecurityTokenHandler();
        var jwt = handler.ReadJwtToken(bearerToken);
        ...
        _logger.LogWarning(
            "Force regeneration requested for {Oid} - bypassing TTL check due to credential error", oid);

        var credentials = await RegenerateCredentialsWithLockAsync(oid, username, bearerToken, isRotation: true, forceRegeneration: true);
        ...
        bool forceRegeneration = false)
    {
        ...
        try
        {
            ...
            // while we were waiting for the lock. Skip this when forceRegeneration is
            // true because the existing KV key is known to be dead on NetApp — returning
            // it would just cause the same credential error again.
        ...
        bool isRotation = false)
    {
        try
        {
            ...
            catch (NetAppNotFoundException ex)
            {
                _logger.LogInformation(ex, "User not found in NetApp, registering new user {UserPrincipalName}", userPrincipalName);
