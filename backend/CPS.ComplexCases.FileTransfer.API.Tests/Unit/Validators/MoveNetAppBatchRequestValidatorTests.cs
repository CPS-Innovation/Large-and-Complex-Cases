...
public class MoveNetAppBatchRequestValidatorTests
{
    ...
    private static MoveNetAppBatchRequest ValidRequest(List<MoveNetAppBatchOperationRequest>? ops = null) => new()
    {
        CaseId = 1,
    ...
    public void Validate_WhenOperationsExceedsMaximum_ReturnsValidationError()
    {
        var ops = Enumerable.Range(1, MoveNetAppBatchRequestValidator.MaxOperations + 1)
            .Select(i => new MoveNetAppBatchOperationRequest { Type = "Material", SourcePath = $"CaseRoot/file{i}.txt" })
            .ToList();
        var result = _validator.TestValidate(ValidRequest(ops));
        result.ShouldHaveValidationErrorFor(x => x.Operations)
            .WithErrorMessage($"A batch may not contain more than {MoveNetAppBatchRequestValidator.MaxOperations} operations.");
    }
    ...
    {
        var ops = new List<MoveNetAppBatchOperationRequest>
        {
    ...
    {
        var request = new MoveNetAppBatchRequest
        {
            CaseId = 1,
