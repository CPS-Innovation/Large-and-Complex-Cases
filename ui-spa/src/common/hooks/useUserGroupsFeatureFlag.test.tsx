...
describe("useUserGroupsFeatureFlag", () => {
  ...
  describe("transfer move feature flag", () => {
    test("Should return transferMove feature false, if FEATURE_FLAG_TRANSFER_MOVE is false for normal user", () => {
      (msalInstanceModule.msalInstance.getAllAccounts as Mock).mockReturnValue([
        {
          username: "test_username",
          name: "test_name",
      ...
      mockConfig.FEATURE_FLAG_TRANSFER_MOVE = false;
      mockConfig.PRIVATE_BETA_FEATURE_USER_GROUP2 =
        "private_beta_feature_group2";
      const { result } = renderHook(() => useUserGroupsFeatureFlag());
      expect(result?.current?.transferMove).toStrictEqual(false);
    });
    test("Should return transferMove feature false, if FEATURE_FLAG_TRANSFER_MOVE is false for automation test user ", () => {
      (auth.useUserDetails as Mock).mockReturnValue({
        username: "dev_user@example.org",
      });
      (msalInstanceModule.msalInstance.getAllAccounts as Mock).mockReturnValue([
    ...
    test("Should return transferMove feature false, if FEATURE_FLAG_TRANSFER_MOVE is true and the user is not added to PRIVATE_BETA_FEATURE_USER_GROUP2 for normal user", () => {
      (msalInstanceModule.msalInstance.getAllAccounts as Mock).mockReturnValue([
        {
          username: "test_username",
          name: "test_name",
      ...
      mockConfig.FEATURE_FLAG_TRANSFER_MOVE = true;
      mockConfig.PRIVATE_BETA_FEATURE_USER_GROUP2 =
        "private_beta_feature_group2";
      const { result } = renderHook(() => useUserGroupsFeatureFlag());
      expect(result?.current?.transferMove).toStrictEqual(false);
    });
    test("Should return transferMove feature true, if FEATURE_FLAG_TRANSFER_MOVE is true and the user is added to PRIVATE_BETA_FEATURE_USER_GROUP2 for normal user", () => {
      (msalInstanceModule.msalInstance.getAllAccounts as Mock).mockReturnValue([
        {
          username: "test_username",
          name: "test_name",
      ...
      mockConfig.PRIVATE_BETA_FEATURE_USER_GROUP2 =
        "private_beta_feature_group2";
      const { result } = renderHook(() => useUserGroupsFeatureFlag());
      expect(result?.current?.transferMove).toStrictEqual(true);
    });
    test("Should return transferMove feature true, if FEATURE_FLAG_TRANSFER_MOVE is true even if the user is not added to PRIVATE_BETA_FEATURE_USER_GROUP2 for automation user", () => {
      (auth.useUserDetails as Mock).mockReturnValue({
        username: "dev_user@example.org",
      });
      (msalInstanceModule.msalInstance.getAllAccounts as Mock).mockReturnValue([
    ...
    test("Should return transferMove feature true, if it is an automation user with search param transfer-move=true, search param take priority for automation user", () => {
      (auth.useUserDetails as Mock).mockReturnValue({
        username: "dev_user@example.org",
      });
      (router.useSearchParams as Mock).mockReturnValue([
        new URLSearchParams("transfer-move=true"),
        vi.fn(),
      ]);
    ...
    test("Should return transferMove feature false, if it is an automation user with search param transfer-move=false,search param take priority for automation user", () => {
      (auth.useUserDetails as Mock).mockReturnValue({
        username: "dev_user@example.org",
      });
      (router.useSearchParams as Mock).mockReturnValue([
        new URLSearchParams("transfer-move=false"),
        vi.fn(),
      ]);
