resource "azurerm_linux_web_app" "complex_cases_proxy" {
  ...
  name                          = "${local.product_name_prefix}-cmsproxy"
  resource_group_name           = azurerm_resource_group.rg_complex_cases.name
  location                      = azurerm_resource_group.rg_complex_cases.location
  service_plan_id               = azurerm_service_plan.asp_complex_cases_proxy.id
...
resource "azuread_application" "complex_cases_proxy" {
  display_name            = "${local.product_name_prefix}-cmsproxy"
  identifier_uris         = ["https://CPSGOVUK.onmicrosoft.com/${local.product_name_prefix}-cmsproxy"]
...
resource "time_rotating" "schedule_proxy" {
  rotation_days = 90
}

resource "azurerm_role_assignment" "ra_blob_data_contributor_complex_cases_proxy" {
  scope                = azurerm_storage_container.complex_cases_proxy_content.resource_manager_id
  role_definition_name = "Storage Blob Data Contributor"
  principal_id         = azurerm_linux_web_app.complex_cases_proxy.identity[0].principal_id
...
resource "azurerm_storage_blob" "nginx_config" {
  ...
  storage_account_name   = azurerm_storage_account.sacpsccproxy.name
  storage_container_name = azurerm_storage_container.complex_cases_proxy_content.name
  type                   = "Block"
  source                 = "nginx.conf"
  depends_on             = [azurerm_role_assignment.ra_blob_data_contributor_complex_cases_proxy]
}
...
resource "azurerm_private_endpoint" "complex_cases_proxy_pe" {
  name                = "${azurerm_linux_web_app.complex_cases_proxy.name}-pe"
  resource_group_name = azurerm_resource_group.rg_complex_cases.name
  location            = azurerm_resource_group.rg_complex_cases.location
  ...
  private_service_connection {
    name                           = "${azurerm_linux_web_app.complex_cases_proxy.name}-psc"
...
resource "azurerm_monitor_diagnostic_setting" "proxy_diagnostic_settings" {
  ...
  target_resource_id             = azurerm_linux_web_app.complex_cases_proxy.id
  ...
  depends_on = [azurerm_linux_web_app.complex_cases_proxy]
}
