resource "azurerm_linux_function_app_slot" "complex_cases_netapp_staging" {
  name                          = "staging"
  function_app_id = azure_linux_function_app.complex_cases_netapp.id
  ...
  app_settings = {
    ...
    "WEBSITE_CONTENTSHARE"                            = azapi_resource.sacpsccnetapp_staging_file_share.name
...
resource "azurerm_private_endpoint" "complex_cases_netapp_staging_pe" {
  name                = "${azurerm_linux_function_app.complex_cases_netapp.name}-staging-pe"
  ...
  private_service_connection {
    name                           = "${azurerm_linux_function_app.complex_cases_netapp.name}-staging-psc"
  ...
  depends_on = [
    azurerm_linux_function_app_slot.complex_cases_netapp_staging
  ]
}