resource "azurerm_linux_web_app" "complex_cases_egressMock" {
  ...
  virtual_network_subnet_id     = azurerm_subnet.sn_complex_cases_egressMock_subnet.id
...
resource "azurerm_private_endpoint" "complex_cases_egressMock_pe" {
  name                = "${azurerm_linux_web_app.complex_cases_egressMock.name}-pe"
  resource_group_name = azurerm_resource_group.rg_complex_cases.name
  location            = azurerm_resource_group.rg_complex_cases.location
  ...
  private_service_connection {
    name                           = "${azurerm_linux_web_app.complex_cases_egressMock.name}-psc"
    private_connection_resource_id = azurerm_linux_web_app.complex_cases_egressMock.id
